...
void HandGesture::addNumberToImg(ImageSource *m){
	...
	for(int i=0;i<numbers2Display.size();i++){
		rectangle(m->original,Point(xPos,yPos),Point(xPos+offset,yPos+offset),numberColor, 2);	
		putText(m->original, intToString(numbers2Display[i]),Point(xPos+7,yPos+offset-3),fontFace,fontSize,numberColor);
		xPos+=40;
...
void HandGesture::drawFingertips(ImageSource *m){
	...
	for(int i=0;i<fingerTips.size();i++){
		p=fingerTips[i];
		putText(m->original,intToString(i),p-Point(0,30),fontFace, 1.2f,Scalar(200,200,200),2);
